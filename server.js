...
app.get("/api/products", async (_req, res) => {
  try {
    const [catRows, prodRows] = await Promise.all([
      ...
      db.query("SELECT id, name, category_id AS category, brand, image, link, price FROM products ORDER BY id"),
    ]);
...
app.post("/api/admin/products", adminAuth, async (req, res) => {
  try {
    const { name, category_id, brand, image, link, price } = req.body;
    if (!name) return res.status(400).json({ error: "Le nom est requis." });
    const { rows } = await db.query(
      `INSERT INTO products (name, category_id, brand, image, link, price)
       VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,
      [name, category_id || null, brand || "", image || "", link || "", price || ""]
    );
    res.json(rows[0]);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.put("/api/admin/products/:id", adminAuth, async (req, res) => {
  try {
    ...
    const { rows } = await db.query(
      `UPDATE products
       SET name=$1, category_id=$2, brand=$3, image=$4, link=$5, price=$6
       WHERE id=$7 RETURNING *`,
      [name, category_id || null, brand || "", image || "", link || "", price || "", req.params.id]
    );
